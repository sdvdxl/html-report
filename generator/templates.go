...
const reportOverviewStartTag = `<aside class="report">`

const specOverviewStartTag = `<aside class="spec">`

const reportOverviewTag = `<div class="report-overview"><h3>Report Overview</h3>
  <div class='box'>
    <div class="chart text_center inline_block_desktop">
      <svg id="pie-chart" data-results="{{.Summary.Failed}},{{.Summary.Passed}},{{.Summary.Skipped}}" data-total="{{.Summary.Total}}">
        <path class="status failedBG" />
        <path class="shadow failedBG" data-status="failed"><title>Failed: {{.Summary.Failed}}/{{.Summary.Total}}</title></path>
        <path class="status passedBG" />
        <path class="shadow passedBG" data-status="passed"><title>Passed: {{.Summary.Passed}}/{{.Summary.Total}}</title></path>
        <path class="status skippedBG" />
        <path class="shadow skippedBG" data-status="skipped"><title>Skipped: {{.Summary.Skipped}}/{{.Summary.Total}}</title></path>
      </svg>
    </div>
    <div class="report_test-results inline_block_desktop">
      <ul>
        <li class="fail" data-status="failed">
          <label>Failed</label>
          <span class="value redtxt">{{.Summary.Failed}}</span>
        </li>
        <li class="pass" data-status="passed">
          <label>Passed</label>
          <span class="value greentxt">{{.Summary.Passed}}</span>
        </li>
        <li class="skip" data-status="skipped">
          <label>Skipped</label>
          <span class="value greytxt">{{.Summary.Skipped}}</span>
        </li>
        <li class="total" data-status="all">
          <label>Total Specs</label>
          <span class="value">{{.Summary.Total}}</span>
        </li>
      </ul>
    </div>
  </div>
  <div class="report_details box">
    <ul>
      <li>
...
<div class='filtersSearch'>
  ...
  <div class="filter">
    <button class='allBG total-specs' data-status="all">All</button>
    <button class='failedBG spec-filter' data-status="failed">Failed</button>
    <button class='passedBG spec-filter' data-status="passed">Passed</button>
    <button class='skippedBG spec-filter' data-status="skipped">Skipped</button>
  </div>
</div>
...
<header class="curr-spec">
  <div class="spec-head-wrapper">
    ...
    <div class='row'>
      <span class='specTime'>{{.ExecTime}}</span>
      <div class="spec_test_results" alt="Scenarios" title="Scenarios">
        <ul>
          <li class="all">
            <label>All</label>
            <span class="value redtxt">{{.Summary.Total}}</span>
          </li>
          <li class="fail">
            <label>Failed</label>
            <span class="value redtxt">{{.Summary.Failed}}</span>
